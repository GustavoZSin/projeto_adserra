...
const inpBase =
  "w-full bg-inp border-[1.5px] rounded-[11px] px-3 py-[10px] text-t1 text-[11px] font-sans outline-none block placeholder:text-t3 transition-all duration-[350ms] focus:border-blue focus:shadow-[0_0_0_3px_var(--blue-sub)]";
const btnP =
  "w-full bg-blue-grad border-none rounded-[11px] py-3 text-white text-[12px] font-bold font-sans cursor-pointer shadow-[0_6px_18px_var(--blue-glow)] tracking-[0.3px] transition-opacity duration-200 flex items-center justify-center gap-[6px] hover:opacity-90 disabled:opacity-60 disabled:cursor-not-allowed";
const btnG =
  "w-full bg-transparent border-[1.5px] border-bdr rounded-[11px] py-[11px] text-t2 text-[12px] font-semibold font-sans cursor-pointer transition-all duration-[350ms] disabled:opacity-60 disabled:cursor-not-allowed";
    ...
    const carregar = async () => {
      ...
      try {
        ...
        if (pub.imagensPublicacao?.length)
          setGaleriaExistente(
            pub.imagensPublicacao.map((img) => ({ id: img.id, url: img.uRL ?? img.url ?? img.URL })),
          );
      } catch {
    ...
    <>
      ...
      <div className="flex flex-col md:hidden h-[calc(100vh-60px)] overflow-hidden">
        <div className="px-[15px] py-[11px] flex items-center justify-between flex-shrink-0 border-b border-bdr2 bg-bg">
          <span className="text-[15px] font-extrabold text-t1">
        ...
        <div className="flex-1 overflow-y-auto min-h-0 px-[14px] py-3 pb-5 scrollbar-hide">
          <p className="text-[11px] text-t3 mb-[14px] leading-[1.5]">
            Compartilhe um evento, ação ou notícia com os docentes.
          </p>
          ...
          <div className="w-full mb-[10px]">
            ...
            <input
              className={clsx(
                inpBase,
                errors.titulo ? "border-red" : "border-bdr",
              )}
              type="text"
            ...
            />
            {errors.titulo && (
              <span className="block text-[9px] text-red mt-[3px]">
                {errors.titulo}
              </span>
            )}
          </div>
          ...
          <div className="w-full mb-[10px]">
            <span className={lbl}>
              Imagem de capa{" "}
              <span className="normal-case text-[8px] font-normal">
                (opcional)
              </span>
            </span>
            <CapaUpload
              preview={capaPreview}
              onClickUpload={() => capaRef.current?.click()}
              onRemove={() => {
                setCapaPreview(null);
                setCapaFile(null);
              }}
              hint="Toque para adicionar imagem"
            />
          </div>
          ...
          <div className="w-full mb-[10px]">
            ...
            <input
              className={clsx(inpBase, "border-bdr")}
              type="date"
              value={data}
          ...
          <div className="w-full mb-4">
            ...
            <div className="relative">
              <input
                className={clsx(inpBase, "border-bdr pr-9")}
                type="text"
                placeholder="Campus Sede FSG"
                value={local}
                onChange={(e) => setLocal(e.target.value)}
              />
              <span className="absolute right-3 top-1/2 -translate-y-1/2 text-icon pointer-events-none flex items-center">
          ...
          {apiError && (
            <div className="bg-red/[0.1] border border-red/25 rounded-[9px] px-3 py-[9px] text-[11px] text-red mb-2">
              {apiError}
            </div>
          )}
      ...
      <div className="hidden md:block py-5 px-[22px]">
        <p className="text-[19px] font-black text-t1 mb-[18px]">
          {isEditing ? "Editar publicação" : "Nova publicação"}
        </p>
          ...
          <div className="flex flex-col gap-3">
            <div className="bg-s1 border border-bdr rounded-[13px] p-[15px]">
              <p className="text-[12px] font-bold text-t1 mb-[10px]">
                Imagem de capa{" "}
                <span className="text-[9px] font-normal text-t3">
                  (opcional)
                </span>
              </p>
              <CapaUpload
    ...
    <div>
      <div className="flex items-center justify-between mb-[7px]">
        <span className={lbl + " mb-0"}>
          Galeria{" "}
          <span className="normal-case text-[8px] font-normal">(opcional)</span>
      ...
      {total > 0 && (
        <div className="flex flex-wrap gap-[6px]">
          {existentes.map((img) => (
            <div
              key={img.id}
              className="relative w-[60px] h-[60px] rounded-[8px] overflow-hidden border border-bdr flex-shrink-0"
            >
              <img
                src={img.url}
                alt=""
              ...
              />
              <span className="absolute bottom-0 left-0 right-0 bg-black/50 text-white text-[7px] text-center py-[2px] font-semibold">
                salva
              </span>
            </div>
          ))}
          {previews.map((url, idx) => (
            <div
              key={idx}
              className="relative w-[60px] h-[60px] rounded-[8px] overflow-hidden border border-[var(--blue-bdr)] flex-shrink-0"
            >
...
const VISIBILIDADE_OPCOES = [
  {
    ...
    label: "Pública",
    Icon: Globe,
    desc: "Visível interna e externamente",
  },
  {
    key: "interna",
    label: "Interna",
    Icon: Users,
    desc: "Só para usuários logados",
  },
];
          ...
          return (
            <button
              ...
              className={clsx(
                ...
                ativo
                  ? "bg-blue-grad text-white shadow-[inset_0_1px_3px_rgba(0,0,0,0.15)]"
...
function RascunhoCheckbox({ rascunho, onChange }) {
  return (
    <label
      className="flex items-center gap-[7px] cursor-pointer select-none mb-3"
