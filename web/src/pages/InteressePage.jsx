...
import { ArrowLeft, Info, Mail, AlertTriangle, CheckCircle2, Check, Loader2 } from 'lucide-react'
...
export default function InteressePage() {
  ...
  const fieldEmail = <Field label="E-mail institucional" value={form.email} onChange={set('email')} placeholder="nome@fsg.edu.br" type="email" icon={<Mail size={13} strokeWidth={1.8} />} error={errors.email} />
    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide px-4 py-[14px] pb-10 md:px-12 md:py-10 md:max-w-[560px] md:mx-auto md:overflow-visible">
        ...
        <button onClick={() => navigate(-1)}
          ...
          <ArrowLeft size={13} strokeWidth={1.8} /> Voltar ao login
        </button>
        ...
        <div className="flex items-start gap-2 bg-[var(--blue-sub)] border border-[var(--blue-bdr)] rounded-[10px] px-3 py-[10px] mb-3.5">
          <span className="text-blue-l flex-shrink-0 mt-px"><Info size={13} strokeWidth={1.8} /></span>
        ...
        <form onSubmit={handleSubmit} noValidate>
          ...
          {errors.autorizaDesconto && (
            <span className="flex items-center gap-[5px] text-[10px] text-red mb-2.5">
              <AlertTriangle size={11} strokeWidth={1.8} style={{ flexShrink: 0 }} /> {errors.autorizaDesconto}
            </span>
          )}
          ...
          <button type="submit" disabled={loading} className={btnPrimary}>
            {loading ? <Loader2 size={15} strokeWidth={1.8} className="animate-spin-fast" /> : 'Enviar solicitação'}
          </button>
