...
const btnBase =
  "inline-flex items-center gap-[5px] px-[13px] py-[7px] rounded-[9px] text-[11px] font-bold font-sans cursor-pointer transition-all duration-200 whitespace-nowrap disabled:opacity-50 disabled:cursor-not-allowed";
...
export default function GerenciarUsuariosPage() {
  ...
  useEffect(() => {
    usuariosService
      .listar()
      .then((r) =>
        setUsuarios(Array.isArray(r.data) ? r.data.map(normalizarUsuario) : []),
      )
      .catch(() => {})
      ...
      <div className="flex flex-col md:hidden h-[calc(100vh-60px)] overflow-hidden">
        <div className="px-[15px] py-[11px] flex items-center justify-between flex-shrink-0 border-b border-bdr2 bg-bg">
          ...
          <div className="flex items-center gap-2">
            <button
              className={clsx(
                "w-[30px] h-[30px] rounded-[9px] flex items-center justify-center border-none cursor-pointer transition-colors",
                mostrarBusca
      ...
      <div className="hidden md:block py-5 px-[22px]">
        <div className="flex items-start justify-between mb-[18px]">
          <div>
            ...
            <p className="text-[11px] text-t3 mt-[3px]">
              Docentes cadastrados no sistema
            </p>
          </div>
  ...
  return (
    <div className="bg-s1 border border-bdr rounded-[13px] overflow-hidden mb-[9px] flex">
      <div
        ...
        style={{ background: "var(--blue-l)" }}
      />
      <div className="flex-1 px-3 py-3 pl-[10px] min-w-0">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-[9px] min-w-0">
            <div
              className="w-9 h-9 rounded-[10px] flex items-center justify-center text-[12px] font-bold text-white flex-shrink-0 font-sans"
            ...
            <div style={{ minWidth: 0 }}>
              ...
              <p className="text-[9px] text-t3 whitespace-nowrap overflow-hidden text-ellipsis">
                {u.email}
              </p>
            </div>
          </div>
          <button
            className={clsx(
              btnBase,
              "bg-red/[0.08] border border-red/30 text-red hover:bg-red/[0.14] flex-shrink-0",
            )}
            onClick={onRemover}
            disabled={removendo}
          >
...
function CelulaInfo({ label, valor }) {
  return (
    <div className="flex flex-col gap-0.5">
      <p className="text-[8px] font-semibold uppercase tracking-[0.5px] text-t3">
        {label}
      </p>
      <p className="text-[11px] font-semibold text-t1 whitespace-nowrap overflow-hidden text-ellipsis">
        {valor || "—"}
      </p>
    </div>
  );
...
function ModalConfirmarRemocao({ nome, onCancelar, onConfirmar }) {
  return (
    <div
      className="fixed inset-0 bg-black/55 backdrop-blur z-[200] flex items-center justify-center p-5"
      onClick={onCancelar}
    >
      <div
        className="bg-s1 border border-bdr rounded-[18px] px-6 py-7 max-w-[340px] w-full flex flex-col items-center text-center shadow-sh"
      ...
      >
        ...
        <p className="text-[11px] text-t3 leading-[1.6] mb-[22px]">
          O acesso de <b className="text-t1">{nome}</b> será removido
